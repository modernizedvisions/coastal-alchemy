...
export async function onRequestPut(context: {
  ...
  try {
    ...
    const videoRow = updated ? await getProductVideoByProductId(context.env.DB, updated.id) : null;
    const product = updated
      ? mapRowToProduct(updated, context.request, context.env, toProductVideoSummary(videoRow))
      : null;
