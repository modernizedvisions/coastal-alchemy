...
interface OrderDetailsModalProps {
  ...
  onClose: () => void;
  onOrderPatched?: (orderId: string, patch: Partial<AdminOrder>) => void;
...
export function OrderDetailsModal({ open, order, onClose, onOrderPatched, onOpenShippingLabels }: OrderDetailsModalProps) {
  ...
  const [overrideSelection, setOverrideSelection] = useState<OrderFulfillmentStatus>('new_order');
  const fulfillmentBadge = getOrderFulfillmentBadgeMeta(order);
  ...
  useEffect(() => {
    const manual = order.manualFulfillmentOverride;
    setOverrideSelection(manual ? manual : fulfillmentBadge.key);
  }, [order.manualFulfillmentOverride, order.id, fulfillmentBadge.key]);

  const applyFulfillmentPatch = (patch: Partial<AdminOrder>) => {
    if (!order?.id) return;
  ...
  useEffect(() => {
    let cancelled = false;
    if (!order?.id || order.viewedAt || order.seenAt || order.isSeen) return () => undefined;
    const markViewed = async () => {
      try {
        const viewed = await adminMarkOrderViewed(order.id);
        if (cancelled) return;
        onOrderPatched?.(order.id, {
          isSeen: viewed.isSeen,
          seenAt: viewed.viewedAt,
          viewedAt: viewed.viewedAt,
        });
      } catch {
        // keep modal open even if viewed tracking fails
      }
    };
    void markViewed();
    return () => {
  ...
  }, [order?.id, order?.viewedAt, order?.seenAt, order?.isSeen, onOrderPatched]);

  const handleSetFulfillment = async (next: OrderFulfillmentStatus) => {
    if (!order?.id) return;
    setFulfillmentSaving(true);
    setOverrideSelection(next);
    try {
      const updated = await adminUpdateOrderFulfillment(order.id, {
        manualFulfillmentOverride: next,
        markAsShipped: next === 'shipped',
      });
      applyFulfillmentPatch({
        isSeen: updated.isSeen,
        seenAt: updated.viewedAt,
        viewedAt: updated.viewedAt,
        manualFulfillmentOverride: updated.manualFulfillmentOverride,
        shippingLabelCreatedAt: updated.shippingLabelCreatedAt,
        fulfillmentShippedAt: updated.fulfillmentShippedAt,
        hasShippingLabelEvidence: updated.hasShippingLabelEvidence,
        fulfillmentStatus: updated.fulfillmentStatus,
      });
      setOverrideSelection(updated.manualFulfillmentOverride ? updated.manualFulfillmentOverride : updated.fulfillmentStatus);
    } catch (error) {
      console.error('Failed to update fulfillment status', error);
      setOverrideSelection(order.manualFulfillmentOverride ? order.manualFulfillmentOverride : fulfillmentBadge.key);
    } finally {
          ...
          <div className="grid grid-cols-1 gap-4">
            ...
            <section className="lux-panel p-3">
              ...
              <div className="mt-2 grid grid-cols-2 gap-2">
                ...
                >
                  {fulfillmentSaving ? 'Saving...' : 'Mark as Shipped'}
                </button>
                <select
                  ...
                  disabled={fulfillmentSaving}
                  onChange={(event) =>
                    void handleSetFulfillment(event.target.value as OrderFulfillmentStatus)
                  }
                >
                  <option value="new_order">New Order</option>
                  <option value="label_needed">Label Needed</option>
