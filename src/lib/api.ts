...
import { emptyProductVideoSummary } from './productVideo';
...
export const adminReorderProducts = reorderAdminProducts;
...
export type { ProductVideoStatus } from './types';

export type AdminProductVideoDetails = {
  provider: string;
...
export type AdminProductVideoResponse = {
  productVideo: ProductVideoSummary;
  details: AdminProductVideoDetails | null;
  warning?: string | null;
};

export type AdminProductVideoDirectUpload = {
...
export type AdminVideoDebugPayload = {
...
const truncateApiText = (value: string, maxLength = 600): string | null => {
...
const readJsonAndSnippet = async <T>(
  response: Response
): Promise<{ payload: T | null; rawTextSnippet: string | null }> => {
  const rawText = await response.text().catch(() => '');
  const rawTextSnippet = truncateApiText(rawText);
  if (!rawTextSnippet) {
    return { payload: null, rawTextSnippet: null };
  }
  try {
    return {
...
type AdminProductVideoResponsePayload = AdminApiErrorPayload & {
  productVideo?: ProductVideoSummary;
  details?: AdminProductVideoDetails | null;
  warning?: string | null;
};

type AdminProductVideoUploadPayload = AdminApiErrorPayload & {
  upload?: AdminProductVideoDirectUpload;
};

type AdminVideoDebugResponsePayload = AdminApiErrorPayload & {
  logged?: boolean;
};

const resolveAdminErrorMessage = (
  response: Response,
  ...
  const detail =
    ...
    payload?.error ||
    payload?.detail ||
    payload?.responseSnippet ||
    payload?.code ||
    rawTextSnippet ||
    fallback;
  const stage = payload?.stage ? `[${payload.stage}] ` : '';
  const upstreamStatusValue = Number.isFinite(Number(payload?.upstreamStatus))
    ? Number(payload?.upstreamStatus)
    : Number.isFinite(Number(payload?.httpStatus))
    ? Number(payload?.httpStatus)
    : null;
  const upstreamStatus = upstreamStatusValue !== null
    ? `, upstream ${upstreamStatusValue}`
    : '';
  return `${stage}${fallback} (${response.status}${upstreamStatus}): ${detail}`;
};

export async function adminFetchProductVideo(
  productId: string,
  options?: { uploadAttemptId?: string | null }
): Promise<AdminProductVideoResponse> {
  const params = new URLSearchParams();
  const uploadAttemptId = (options?.uploadAttemptId || '').trim();
  if (uploadAttemptId) params.set('uploadAttemptId', uploadAttemptId);
  const query = params.toString();
  const path = `/api/admin/products/${encodeURIComponent(productId)}/video${query ? `?${query}` : ''}`;
  const response = await adminFetch(path, {
    method: 'GET',
    headers: { Accept: 'application/json' },
    cache: 'no-store',
  });
  const { payload: data, rawTextSnippet } = await readJsonAndSnippet<AdminProductVideoResponsePayload>(response);
  if (!response.ok || data?.ok === false) {
    throw new Error(resolveAdminErrorMessage(response, data, 'Fetch product video failed', rawTextSnippet));
  }
  return {
    productVideo: (data?.productVideo as ProductVideoSummary) || emptyProductVideoSummary(),
    details: (data?.details as AdminProductVideoDetails | null) || null,
    warning: (data?.warning as string | null | undefined) || null,
  };
}

export async function adminCreateProductVideoUpload(
  productId: string,
  input: {
    filename: string;
    mimeType: string;
  ...
  const response = await adminFetch(
    `/api/admin/products/${encodeURIComponent(productId)}/video/create-upload`,
    {
      method: 'POST',
      headers: { 'Content-Type': 'application/json', Accept: 'application/json' },
  ...
  const { payload: data, rawTextSnippet } = await readJsonAndSnippet<AdminProductVideoUploadPayload>(response);
  if (!response.ok || data?.ok === false) {
    const error = new Error(
      resolveAdminErrorMessage(response, data, 'Create video upload failed', rawTextSnippet)
    ) as Error & {
      httpStatus?: number | null;
    ...
    error.upstreamHttpStatus = Number.isFinite(Number(data?.upstreamStatus))
      ? Number(data?.upstreamStatus)
      : Number.isFinite(Number(data?.httpStatus))
      ? Number(data?.httpStatus)
      : null;
    error.responseSnippet = data?.responseSnippet || rawTextSnippet || null;
    error.stage = data?.stage || null;
    throw error;
  }
  const upload = data?.upload as AdminProductVideoDirectUpload | undefined;
  if (!upload?.uploadUrl || !upload?.providerAssetId) {
    throw new Error('Create video upload failed: missing upload payload');
  }
  return { upload };
}

export async function adminAttachProductVideo(
  productId: string,
  input: {
    providerAssetId: string;
    filename: string;
  ...
  const response = await adminFetch(`/api/admin/products/${encodeURIComponent(productId)}/video/attach`, {
    method: 'POST',
    headers: { 'Content-Type': 'application/json', Accept: 'application/json' },
    body: JSON.stringify(input),
  });
  const { payload: data, rawTextSnippet } = await readJsonAndSnippet<AdminProductVideoResponsePayload>(response);
  if (!response.ok || data?.ok === false) {
    throw new Error(resolveAdminErrorMessage(response, data, 'Attach product video failed', rawTextSnippet));
  }
  return {
    productVideo: (data?.productVideo as ProductVideoSummary) || emptyProductVideoSummary(),
    details: (data?.details as AdminProductVideoDetails | null) || null,
...
export async function adminReplaceProductVideo(
  productId: string,
  input: {
    providerAssetId: string;
    filename: string;
  ...
  const response = await adminFetch(`/api/admin/products/${encodeURIComponent(productId)}/video/replace`, {
    method: 'POST',
    headers: { 'Content-Type': 'application/json', Accept: 'application/json' },
    body: JSON.stringify(input),
  });
  const { payload: data, rawTextSnippet } = await readJsonAndSnippet<AdminProductVideoResponsePayload>(response);
  if (!response.ok || data?.ok === false) {
    throw new Error(resolveAdminErrorMessage(response, data, 'Replace product video failed', rawTextSnippet));
  }
  return {
    productVideo: (data?.productVideo as ProductVideoSummary) || emptyProductVideoSummary(),
    details: (data?.details as AdminProductVideoDetails | null) || null,
...
export async function adminRemoveProductVideo(
  productId: string,
  options?: { uploadAttemptId?: string | null }
): Promise<void> {
  const params = new URLSearchParams();
  ...
  const { payload: data, rawTextSnippet } = await readJsonAndSnippet<AdminApiErrorPayload>(response);
  if (!response.ok || data?.ok === false) {
    throw new Error(resolveAdminErrorMessage(response, data, 'Remove product video failed', rawTextSnippet));
  }
}

export async function adminSendVideoDebugBeacon(payload: AdminVideoDebugPayload): Promise<void> {
  const response = await adminFetch('/api/admin/video-debug', {
    method: 'POST',
    headers: { 'Content-Type': 'application/json', Accept: 'application/json' },
    body: JSON.stringify(payload),
  });
  const { payload: data, rawTextSnippet } = await readJsonAndSnippet<AdminVideoDebugResponsePayload>(response);
  if (!response.ok || data?.ok === false) {
    throw new Error(resolveAdminErrorMessage(response, data, 'Video debug beacon failed', rawTextSnippet));
  }
}
