...
export const onRequestPost = async (context: {
  ...
  try {
    switch (event.type) {
      ...
      const metadataProductIds = (session.metadata?.mv_product_ids || '')
        .split(',')
        .map((value) => value.trim())
        .filter((value) => value.length > 0);
      const fallbackSessionCanonicalProductId = metadataProductIds.length === 1 ? metadataProductIds[0] : null;
      const productId = session.metadata?.product_id || fallbackSessionCanonicalProductId;
      ...
      if (invoiceId) {
        ...
        let missingCanonicalMetadataCount = 0;
        for (const line of lineItems) {
          ...
          const metadataStripeProductId = meta.sourceStripeProductId ? meta.sourceStripeProductId.trim() : '';
          const metadataStripePriceId = meta.sourceStripePriceId ? meta.sourceStripePriceId.trim() : '';
          const metadataProductSlug = meta.sourceProductSlug ? meta.sourceProductSlug.trim() : '';
          if (!metadataProductId) missingCanonicalMetadataCount += 1;
          ...
          const key =
            ...
            metadataStripePriceId ||
            metadataProductSlug ||
            productIdFromPrice ||
            priceId ||
            fallbackSessionProductId;
          ...
          const source = metadataProductId
            ? 'metadata'
            : metadataStripeProductId
            ? 'metadata_stripe_product'
            : metadataStripePriceId
            ? 'metadata_stripe_price'
            : metadataProductSlug
            ? 'metadata_slug'
            : productIdFromPrice
            ? 'stripe_product'
        ...
        if (missingCanonicalMetadataCount > 0) {
          console.warn('[sold-update] line items missing canonical dd_product_id metadata', {
            sessionId: session.id,
            missingCanonicalMetadataCount,
            lineItemCount: lineItems.length,
            hasSingleMetadataFallback: !!fallbackSessionCanonicalProductId,
          });
        }
...
async function ensureOrdersSchema(db: D1Database) {
  ...
  const addColumnIfMissing = async (name: string, ddl: string) => {
    ...
    await addColumnIfMissing('viewed_at', `ALTER TABLE orders ADD COLUMN viewed_at TEXT;`);
    ...
    await addColumnIfMissing('shipping_label_created_at', `ALTER TABLE orders ADD COLUMN shipping_label_created_at TEXT;`);
    await addColumnIfMissing('fulfillment_shipped_at', `ALTER TABLE orders ADD COLUMN fulfillment_shipped_at TEXT;`);
...
async function handleCustomInvoicePayment(args: {
  ...
  const siteUrl = (env.PUBLIC_SITE_URL || env.VITE_PUBLIC_SITE_URL || '').replace(/\/+$/, '');
  const invoiceLinkBase = siteUrl ? `${siteUrl}/invoice/${invoiceId}` : `/invoice/${invoiceId}`;
  const invoiceLink = buildTrackedEmailSiteUrl(invoiceLinkBase, 'custom_order_followup', {
    siteOrigin: siteUrl || null,
  });
  const adminUrl = siteUrl ? `${siteUrl}/admin` : '/admin';
  const ownerTo = env.RESEND_OWNER_TO || env.EMAIL_OWNER_TO;
...
async function insertStandardOrderAndItems(args: {
  ...
  const giftPromotionId = trimOrNull(session.metadata?.gift_promotion_id);
  const giftProductId = trimOrNull(session.metadata?.gift_product_id);
  const rawGiftQuantity = typeof session.metadata?.gift_quantity === 'string' ? session.metadata.gift_quantity.trim() : '';
  const parsedGiftQuantity = Number.parseInt(rawGiftQuantity, 10);
  const giftQuantity = Number.isFinite(parsedGiftQuantity) && parsedGiftQuantity > 0 ? parsedGiftQuantity : null;
    ...
    .prepare(
      `
        INSERT INTO orders (
          ...
          gift_promotion_id, gift_product_id, gift_quantity
        ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?);
      `
    )
    .bind(
      ...
      : await (async () => {
        ...
        for (const line of lines) {
          ...
          const resolvedProductId =
            meta.sourceProductId ||
            meta.sourceStripeProductId ||
            meta.sourceStripePriceId ||
            meta.sourceProductSlug ||
            productIdFromPrice ||
            productId ||
            line.price?.id ||
            'unknown';
...
function extractOptionMetadata(line: Stripe.LineItem): {
  ...
  const rawStripePrice = meta?.dd_stripe_price_id;
  const rawSlug = meta?.dd_product_slug;
  return {
    ...
    sourceStripeProductId: typeof rawStripeSource === 'string' && rawStripeSource.trim().length ? rawStripeSource.trim() : null,
