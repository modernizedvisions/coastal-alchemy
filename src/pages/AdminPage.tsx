...
import { adminMarkOrderViewed } from '../lib/adminOrders';
...
const reorderByIds = (products: Product[], orderedIds: string[]): Product[] => {
  ...
  if (reordered.length === products.length) return reordered;

  const included = new Set(reordered.map((product) => product.id));
  products.forEach((product) => {
    if (!included.has(product.id)) reordered.push(product);
  });
  return reordered;
};
...
export function AdminPage() {
  ...
  const handleSelectOrder = (order: AdminOrder) => {
    ...
    if (order.viewedAt || order.seenAt || order.isSeen) return;
    const optimisticViewedAt = new Date().toISOString();
    applyOrderPatch(order.id, {
      isSeen: true,
      seenAt: optimisticViewedAt,
  ...
  const applyOrderPatch = (orderId: string, patch: Partial<AdminOrder>) => {
    setOrders((prev) => prev.map((o) => (o.id === orderId ? { ...o, ...patch } : o)));
    setSelectedOrder((prev) => (prev && prev.id === orderId ? { ...prev, ...patch } : prev));
    setShippingModalOrder((prev) => (prev && prev.id === orderId ? { ...prev, ...patch } : prev));
  };

  const markOrderSeen = async (orderId: string) => {
    try {
      const data = await adminMarkOrderViewed(orderId);
      applyOrderPatch(orderId, {
        isSeen: data.isSeen,
        seenAt: data.viewedAt,
        viewedAt: data.viewedAt,
      });
      setUnseenOrders(data.unseenCount);
    } catch (err) {
      console.error('Failed to mark order as seen', err);
      applyOrderPatch(orderId, { isSeen: false, seenAt: null, viewedAt: null });
  ...
  const handleCreateProduct = async (e: React.FormEvent): Promise<Product | null> => {
    e.preventDefault();
  ...
  const handleReorderProducts = async (orderedIds: string[]) => {
    ...
    try {
      await adminReorderProducts(orderedIds);
      await loadAdminProducts();
    } catch (err) {
      console.error('Reorder products failed', err);
      setAdminProducts(previous);
      setProductStatus({ type: 'error', message: 'Could not save product order.' });
    }
  };
      ...
      <div className="w-full max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        {activeTab === 'shop' && (
          <AdminShopTab
            ...
            onReorderEditImages={(orderedIds) => reorderImagesByIds(orderedIds, setEditProductImages)}
        ...
        <OrderDetailsModal
          ...
          onOrderPatched={(orderId, patch) => applyOrderPatch(orderId, patch)}
