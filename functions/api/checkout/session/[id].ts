  ...
  try {
    ...
    const sourceProductIds = lineItemsRaw
      .map((li) => {
        ...
        const source = meta?.dd_canonical_product_id || meta?.dd_product_id;
        return typeof source === 'string' && source.trim().length > 0 ? source.trim() : null;
      })
      .filter(Boolean) as string[];
    const sourceProductSlugs = lineItemsRaw
      .map((li) => {
        const meta = extractLineItemMetadata(li);
        const source = meta?.dd_product_slug;
        return typeof source === 'string' && source.trim().length > 0 ? source.trim() : null;
      })
      .filter(Boolean) as string[];
    ...
    if (env.DB && (stripeProductIds.length || stripePriceIds.length || sourceProductIds.length || sourceProductSlugs.length)) {
      ...
      const placeholdersSlug = sourceProductSlugs.map(() => '?').join(',');
      const whereClauses = [];
      ...
      if (placeholdersSlug) {
        whereClauses.push(`slug IN (${placeholdersSlug})`);
        bindValues.push(...sourceProductSlugs);
      }
      try {
        const query = `
          SELECT id, name, slug, category, stripe_product_id, stripe_price_id, image_url, image_urls_json, is_one_off
          FROM products
        ...
        (results || []).forEach((row) => {
          ...
          if (row.slug) productLookup.set(row.slug, row);
        });
      } catch (dbError) {
      ...
      if (names.length) {
        ...
        try {
          const { results } = await env.DB
            .prepare(
              `
              SELECT id, name, slug, category, stripe_product_id, stripe_price_id, image_url, image_urls_json, is_one_off, price_cents
              FROM products
    ...
    const lineItems =
      lineItemsRaw.map((li) => {
        ...
        const canonicalIdRaw =
          typeof meta?.dd_canonical_product_id === 'string'
            ? meta.dd_canonical_product_id
            : typeof meta?.dd_product_id === 'string'
            ? meta.dd_product_id
            : '';
        const canonicalProductId =
          canonicalIdRaw.trim().length
            ? canonicalIdRaw.trim()
            : null;
        const metadataStripeProductId =
          typeof meta?.dd_stripe_product_id === 'string' && meta.dd_stripe_product_id.trim().length
            ? meta.dd_stripe_product_id.trim()
            : null;
        const metadataStripePriceId =
          typeof meta?.dd_stripe_price_id === 'string' && meta.dd_stripe_price_id.trim().length
            ? meta.dd_stripe_price_id.trim()
            : null;
        const metadataProductSlug =
          typeof meta?.dd_product_slug === 'string' && meta.dd_product_slug.trim().length
            ? meta.dd_product_slug.trim()
            : null;
        const stripeProductIdFromPrice =
          typeof li.price?.product === 'string'
            ? li.price?.product
            : li.price?.product && typeof li.price.product === 'object'
            ? (li.price.product as Stripe.Product).id
            : null;
        const stripePriceIdFromPrice = typeof li.price?.id === 'string' ? li.price.id : null;
        const stripeProductId = metadataStripeProductId || stripeProductIdFromPrice;
        const stripePriceId = metadataStripePriceId || stripePriceIdFromPrice;
        const productName =
          (li.price?.product &&
            typeof li.price.product !== 'string' &&
        ...
        const metaMatch = canonicalProductId ? productLookup.get(canonicalProductId) : null;
        ...
        const slugMatch = !metaMatch && !keyMatch && !priceMatch && metadataProductSlug ? productLookup.get(metadataProductSlug) : null;
        let matchedProduct = metaMatch || keyMatch || priceMatch || slugMatch || null;
        ...
        return {
          ...
          isShipping,
          productId: matchedProduct?.id || canonicalProductId || null,
          stripeProductId,
          stripePriceId,
          productSlug: matchedProduct?.slug || metadataProductSlug || null,
          optionGroupLabel,
          optionValue,
        };
      }) ?? [];
