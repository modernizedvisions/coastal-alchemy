...
import { fetchActiveGiftPromotionRow } from '../_lib/giftPromotions';
...
const toMetadataListValue = (values: string[]): string => values.join(',').slice(0, 450);
...
export const onRequestPost = async (context: {
  ...
  try {
    ...
    const activeGiftPromotion = (await fetchActiveGiftPromotionRow(env.DB, nowIso)) as GiftPromotionRow | null;
    ...
    const canonicalProductIdsForSession = new Set<string>();
    let subtotalCents = 0;
    ...
    for (const item of groupedItems) {
      ...
      canonicalProductIdsForSession.add(product.id);
    ...
    let appliedGiftPromotionId = '';
    let appliedGiftProductId = '';
    let appliedGiftQuantity = 0;

    if (activeGiftPromotion) {
      try {
        const thresholdSubtotalCents = Math.max(1, Number(activeGiftPromotion.threshold_subtotal_cents || 0));
        const giftProductId = (activeGiftPromotion.gift_product_id || '').trim();
        const giftQuantity = Math.max(1, Math.floor(Number(activeGiftPromotion.gift_quantity || 1)));

        if (thresholdSubtotalCents > 0 && giftProductId && subtotalCents >= thresholdSubtotalCents) {
          const giftProduct = await env.DB
            .prepare(
              `
                SELECT id, name, slug, description, price_cents, category, image_url, image_urls_json, is_active,
                       is_one_off, is_sold, quantity_available, stripe_price_id, stripe_product_id, collection,
          ...
          if (!giftProduct) {
            console.warn('[checkout gift] gift product not found, skipping gift line', {
              giftPromotionId: activeGiftPromotion.id,
              giftProductId,
            });
          } else {
            const giftImageUrls = resolveProductImages(giftProduct).filter((url) => {
              if (isAbsoluteHttpUrl(url)) return true;
              return false;
            });
            const giftMetadata: Stripe.MetadataParam = {
              dd_canonical_product_id: giftProduct.id,
              dd_product_id: giftProduct.id,
              dd_is_gift: '1',
              dd_gift_promotion_id: activeGiftPromotion.id,
              dd_gift_product_id: giftProduct.id,
            };
            if (giftProduct.stripe_product_id) giftMetadata.dd_stripe_product_id = giftProduct.stripe_product_id;
            if (giftProduct.stripe_price_id) giftMetadata.dd_stripe_price_id = giftProduct.stripe_price_id;
            if (giftProduct.slug) giftMetadata.dd_product_slug = giftProduct.slug;
            lineItems.push({
              price_data: {
                currency: 'usd',
                unit_amount: 0,
                product_data: {
                  ...
                  images: giftImageUrls.length ? giftImageUrls.slice(0, 4) : undefined,
                  metadata: giftMetadata,
                },
              },
              quantity: giftQuantity,
            });
            appliedGiftPromotionId = activeGiftPromotion.id;
            appliedGiftProductId = giftProduct.id;
            appliedGiftQuantity = giftQuantity;
          }
        }
      } catch (giftError) {
        console.error('[checkout gift] failed to evaluate gift promotion, continuing without gift', giftError);
      }
    }
    ...
    try {
      ...
      const session = await createCheckoutSession(stripeSecretKey, {
        ...
        metadata: {
          mv_product_ids: toMetadataListValue(Array.from(canonicalProductIdsForSession)),
          ...
          gift_promotion_id: appliedGiftPromotionId,
          gift_product_id: appliedGiftProductId,
          gift_quantity: String(appliedGiftQuantity),
        },
        consent_collection: {
          promotions: 'auto',
        },
