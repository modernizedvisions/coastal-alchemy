...
const IMAGE_LOOKUP_CHUNK_SIZE = 200;
...
export async function resolveImageIdsToUrls(
  ...
  for (const chunk of chunkArray(unique, IMAGE_LOOKUP_CHUNK_SIZE)) {
...
export async function resolveImageUrlsToIds(
  ...
  for (const keyChunk of chunkArray(storageKeys, IMAGE_LOOKUP_CHUNK_SIZE)) {
    const placeholders = keyChunk.map(() => '?').join(', ');
    const { results } = await db
      .prepare(`SELECT id, storage_key, public_url FROM images WHERE storage_key IN (${placeholders});`)
      .bind(...keyChunk)
      .all<ImageRow>();
  ...
  for (const urlChunk of chunkArray(normalized, IMAGE_LOOKUP_CHUNK_SIZE)) {
    const placeholders = urlChunk.map(() => '?').join(', ');
    const { results } = await db
      .prepare(`SELECT id, storage_key, public_url FROM images WHERE public_url IN (${placeholders});`)
      .bind(...urlChunk)
      .all<ImageRow>();
